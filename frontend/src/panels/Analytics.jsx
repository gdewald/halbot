...
function BarRow({ rank, label, count, max, last, accent, mono, onClick, active, avatar, emoji, emojiIndex }) {
  const pct = max > 0 ? Math.max(4, (count / max) * 100) : 0;
  const hasIcon = !!emoji || avatar !== undefined;
    ...
    >
      ...
      {emoji ? (
        <span style={{
          ...
          <EmojiCell raw={emoji} emojiIndex={emojiIndex} size={16} />
        </span>
      ) : avatar !== undefined && (
        <span style={{
...
export function AnalyticsPanel() {
  ...
  // Emoji list — loaded once. Used by EmojiCell to render custom
  // Discord emoji `<:name:id>` as <img> against image_data_url.
  useEffect(() => {
    let cancelled = false;
    b.emojiList().then(list => {
      if (!cancelled) setEmojis(Array.isArray(list) ? list : []);
    }).catch(() => { /* stub */ });
          ...
          <div style={{
            ...
            {topSounds.rows.length === 0 ? (
              ...
              <BarRow key={r.key} rank={soundsPg.page * soundsPg.pageSize + i + 1} label={r.key || '—'}
                ...
                emoji={r.emoji || ''} emojiIndex={emojiIndex}
