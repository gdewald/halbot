...
- **Channel history**: 50 messages are passed as conversation context. Bot's own replies are **skipped entirely** — feeding them back (even with a `[BOT REPLY]` prefix warning) caused the LLM to mimic its prior user-facing prose instead of returning JSON actions, breaking action dispatch (e.g. bot would say "Joined voice" without actually connecting).
