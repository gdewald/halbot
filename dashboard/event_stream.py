"""Consume StreamEvents RPC; buffer for pull-style frontend polling."""
        ...
        while not self._stop.is_set():
            try:
                with grpc.insecure_channel(self._target) as ch:
                    ...
                    req = mgmt_pb2.StreamEventsRequest(backlog=50, kind="", user_id=0)
                    for ev in stub.StreamEvents(req):
                        if self._stop.is_set():
                            break
                        rec = {
                            "ts_ns": int(ev.ts_unix_nanos),
                            "kind": ev.kind,
                ...
                log.info("event stream disconnect (%s); retrying in 2s", code)
                time.sleep(2.0)
